...
void Bullet::ShootBullet(float t) {
	...
	auto PlanePos = MyPlane::sharedPlane->getChildByTag(AIRPLANE)->getPosition();
	//从缓存中创建子弹
	auto spritebullet = Sprite::createWithSpriteFrameName("bullet1.png");
	...
	Point bulletPos = (Point(PlanePos.x,
		PlanePos.y
		+ MyPlane::sharedPlane->getChildByTag(AIRPLANE)->getContentSize().height
		/ 2 + 20)); 
